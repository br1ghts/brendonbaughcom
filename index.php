				...
				<?php if ( $slider_posts ) : ?>
					<section class="space-y-4 w-full">
						...
						<div class="relative w-full" role="region" aria-label="<?php esc_attr_e( 'Featured posts slider', 'brendon-core' ); ?>">
							<div data-slider-track class="flex w-full gap-4 overflow-x-auto pb-2 scroll-smooth snap-x snap-mandatory touch-pan-x">
								<?php foreach ( $slider_posts as $post ) : ?>
									<?php setup_postdata( $post ); ?>
									<?php $thumb_url = has_post_thumbnail( $post ) ? get_the_post_thumbnail_url( $post, 'large' ) : ''; ?>
									<article class="relative min-w-full snap-start rounded-3xl border border-[#F2A25C]/30 bg-white p-6 shadow-lg transition hover:-translate-y-1 hover:shadow-2xl dark:border-[#F2A25C]/20 dark:bg-slate-900 animate-featuredWave">
										...
										<?php if ( $thumb_url ) : ?>
											<div class="mb-4 overflow-hidden rounded-2xl bg-slate-100">
												<img class="h-52 w-full object-cover transition duration-300 group-hover:scale-105" src="<?php echo esc_url( $thumb_url ); ?>" alt="<?php echo esc_attr( get_the_title( $post ) ); ?>" />
											</div>
										<?php endif; ?>
				...
				<?php if ( have_posts() ) : ?>
					...
					<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
						...
						while ( have_posts() ) :
							...
							?>
							<article class="<?php echo esc_attr( "relative flex flex-col gap-4 rounded-3xl border border-[#F2A25C]/30 bg-white p-6 shadow-lg transition hover:-translate-y-0.5 hover:shadow-2xl dark:border-[#F2A25C]/20 dark:bg-slate-900 dark:text-slate-100 {$span_class}" ); ?>">
								...
								<header class="flex flex-col gap-2">
									...
									<h3 class="text-xl font-semibold leading-tight">
										<a class="text-slate-900 transition-colors hover:text-[#F26D3D] dark:text-white" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
									</h3>
